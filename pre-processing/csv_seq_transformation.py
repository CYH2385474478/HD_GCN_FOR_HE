...
#labels_vector = np.zeros((num_skes, 2))记得要改这里的2是指的动作个数
...
def seq_translation(skes_joints):
    ...
    for idx, ske_joints in enumerate(skes_joints):
        num_frames = ske_joints.shape[0]
        num_bodies = 1 if ske_joints.shape[1] == 120 else 2  # Change 75 to 120
        if num_bodies == 2:
        ...
        for f in range(num_frames):
            if num_bodies == 1:
                ske_joints[f] -= np.tile(origin, 40)  # Change 25 to 40
            else:  # for 2 actors
                ske_joints[f] -= np.tile(origin, 80)  # Change 50 to 80
...
def frame_translation(skes_joints, skes_name, frames_cnt):
    ...
    for idx, ske_joints in enumerate(skes_joints):
        ...
        j21 = ske_joints[:, 111:114]  # Change 60:63 to 111:114
        dist = np.sqrt(((j1 - j21) ** 2).sum(axis=1))

        for f in range(num_frames):
            origin = ske_joints[f, 111:114]  # new origin: middle of the spine (joint-2)
            # if (ske_joints[f, 75:] == 0).all():
            #     ske_joints[f, :75] = (ske_joints[f, :75] - np.tile(origin, 25)) / \
            #                           dist[f] + np.tile(origin, 25)
            if (ske_joints[f, 120:] == 0).all():  # Change 75 to 120
                ske_joints[f, :120] = (ske_joints[f, :120] - np.tile(origin, 40)) / dist[f] + np.tile(origin,
                                                                                                      40)  # Change 25 to 40
...
def align_frames(skes_joints, frames_cnt):
    ...
    aligned_skes_joints = np.zeros((num_skes, max_num_frames, 240), dtype=np.float32)  # Change 150 to 240
...
def one_hot_vector(labels):
    num_skes = len(labels)
    labels_vector = np.zeros((num_skes, 38))
