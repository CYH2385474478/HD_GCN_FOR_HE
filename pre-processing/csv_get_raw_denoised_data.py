...
def get_one_actor_points(body_data, num_frames):
    ...
    For joints, each frame contains 120 X-Y-Z coordinates.
    For colors, each frame contains 40 x 2 (X, Y) coordinates.
    """
    joints = np.zeros((num_frames, 120), dtype=np.float32)
    colors = np.ones((num_frames, 1, 40, 2), dtype=np.float32) * np.nan
    start, end = body_data['interval'][0], body_data['interval'][-1]
    joints[start:end + 1] = body_data['joints'].reshape(-1, 120)
...
def get_two_actors_points(bodies_data):
    ...
    if len(bodies_data) == 1:  # Only left one actor after denoising
        ...
        joints = np.zeros((num_frames, 240), dtype=np.float32)
        colors = np.ones((num_frames, 2, 40, 2), dtype=np.float32) * np.nan

        bodyID, actor1 = bodies_data[0]  # the 1st actor with largest motion
        start1, end1 = actor1['interval'][0], actor1['interval'][-1]
        joints[start1:end1 + 1, :120] = actor1['joints'].reshape(-1, 120)
        ...
        while len(bodies_data) > 0:
            ...
            if min(end1, end) - max(start1, start) <= 0:  # no overlap with actor1
                joints[start:end + 1, :120] = actor['joints'].reshape(-1, 120)
