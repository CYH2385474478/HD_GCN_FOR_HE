...
def get_raw_bodies_data(skes_path, ske_file, frames_drop_skes, frames_drop_logger):
    """
    Get raw bodies data from a skeleton sequence.

    Each body's data is a dict that contains the following keys:
      - joints: raw 3D joints positions. Shape: (num_frames x 40, 3)
      - colors: raw 2D color locations. Shape: (num_frames, 40, 2)
    ...
    """
    ske_file = osp.join(skes_path, ske_file)
    ...
    for f in range(num_frames):
        ...
        valid_frames += 1
        joints = np.zeros((num_bodies, 40, 3), dtype=np.float32)
        colors = np.zeros((num_bodies, 40, 2), dtype=np.float32)
        ...
        for b in range(num_bodies):
            ...
            if bodyID not in bodies_data:  # Add a new body's data
                body_data = dict()
                body_data['joints'] = joints[b]  # ndarray: (40, 3)
                body_data['colors'] = colors[b, np.newaxis]  # ndarray: (1, 40, 2)
    ...
    assert num_frames_drop < num_frames, \
        'Error: All frames data (%d) of %s is missing or lost' % (num_frames, ske_file)
    if num_frames_drop > 0:
        frames_drop_skes[ske_file] = np.array(frames_drop, dtype=int)
        frames_drop_logger.info('{}: {} frames missed: {}\n'.format(ske_file, num_frames_drop,
                                                                    frames_drop))
    ...
    return {'name': ske_file, 'data': bodies_data, 'num_frames': num_frames - num_frames_drop}
...
def get_raw_skes_data():
    ...
    # Get a list of all .skeleton files in the directory
    skes_files = [f for f in os.listdir(skes_path) if f.endswith('.skeleton')]

    num_files = len(skes_files)
    print('Found %d available skeleton files.' % num_files)

    frames_cnt = np.zeros(num_files, dtype=int)

    for (idx, ske_file) in enumerate(skes_files):
        bodies_data = get_raw_bodies_data(skes_path, ske_file, frames_drop_skes, frames_drop_logger)
...
if __name__ == '__main__':
    ...
    skes_path = r'C:\Users\cyh23\Desktop\论文复现\HD-GCN-main\pre-processing\all_skl'
